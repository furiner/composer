...
using osuTK.Input;
using Realms;
using DrawableCarouselItem = composer.Editor.Screens.Select.Carousel.DrawableCarouselItem;
...
namespace composer.Editor.Screens.Select
{
    public partial class BeatmapCarousel : CompositeDrawable, IKeyBindingHandler<GlobalAction>
    {
        ...
        private const float distance_offscreen_to_preload = 512;
        ...
        private readonly List<CarouselItem> visibleItems = new();

        private readonly Cached itemsCache = new();
        private PendingScrollOperation pendingScrollOperation = PendingScrollOperation.None;

        public readonly Bindable<bool> RightClickScrollingEnabled = new();

        public readonly Bindable<RandomSelectAlgorithm> RandomAlgorithm = new();
        private readonly List<CarouselBeatmapSet> previouslyVisitedRandomSets = new();
        private readonly Stack<CarouselBeatmap> randomSelectedBeatmaps = new();
        ...
        private readonly DrawablePool<DrawableCarouselBeatmapSetCard> setPool = new(100);
        ...
        private void beatmapsChanged(IRealmCollection<BeatmapInfo> sender, ChangeSet? changes, Exception? error)
        {
            ...
            foreach (int i in changes.InsertedIndices)
            {
                ...
                if (root.BeatmapSetsByID.TryGetValue(beatmapSet.ID, out var existingSet) && existingSet.BeatmapSet.Beatmaps.All(b => b.ID != beatmapInfo.ID))
                {
                ...
                switch (item)
                {
                    case CarouselBeatmapSet set:
                    {
                        ...
                        if (item.State.Value == CarouselItemState.Selected)
                        {
                            ...
                            scrollTarget = currentY + DrawableCarouselBeatmapSetCard.HEIGHT - visibleHalfHeight + BleedTop;
        ...
        {
            public override osu.Game.Screens.Select.Carousel.DrawableCarouselItem? CreateDrawableRepresentation() => throw new NotImplementedException();
        }
        ...
        private class CarouselRoot : CarouselGroupEagerSelect
        {
            ...
            public readonly Dictionary<Guid, CarouselBeatmapSet> BeatmapSetsByID = new();
